...
private:
    ...
    int h;
    int p_num;
    int valid[maxn],p[maxn],visit[maxn],en[maxn],en2[maxn];
    char gang[10];
    Expression Re;
};
...
void Exp::errase(int start,int end)
{
    valid[start] = 1;
    p[++h] = start;
    en[end] = 1;
...
int main()
{
    ...
    {
        gets(text);
        if(re.match(text))puts("match");else puts("error");
    }
    return 0;
}
